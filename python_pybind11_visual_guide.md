...
```text
RuntimeError: Tried to call pure virtual function "AnimalPure::sound"
```

所以 pure virtual 版本比普通 trampoline 更严格：

- 普通 `PYBIND11_OVERRIDE`：没 override 时还能 fallback
- `PYBIND11_OVERRIDE_PURE`：没 override 就直接报错
...
## 20.18 最小完整示例：把整条链压缩成 6 段代码

如果你只想抓住 trampoline 的最小骨架，可以只记下面这 6 段代码。

### 1) C++ 基类：定义 virtual 接口
...
### 2) trampoline 类：把 virtual 调用桥到 Python
...
### 4) pybind11 绑定：告诉 pybind11 这个类要配 trampoline
...
```text
C++ 的 virtual dispatch
真的经由 trampoline 回调到了 Python override
```

如果这 6 段你都吃透了，trampoline 的主干就已经掌握了。
...
这里的“碰 Python 运行时”不只是“执行 Python 代码”，还包括：
...
所以在 pybind11 语境下，GIL 保护的不只是“字节码执行权”，而是整个 CPython 对象世界的一致性。

你可以先把这一章压缩成一句口令：

```text
碰 Python，就拿着；跑纯 C++，才考虑放下。
```
...
把一次 pybind11 调用想成穿过三段区域：

```text
Python caller
   │
   ▼
[区域 A] Python / pybind11 边界层
   - 参数解析
   - Python 对象访问
   - 返回值封装
   => 必须持有 GIL
...
这个三段图比“背很多零碎规则”更稳，因为绝大多数场景都能归约到这里。
...
```text
“创建、访问、修改、释放 Python 对象，都可能需要 GIL”
```

因为一个 `py::object` 的析构，往往就意味着要改引用计数；而改引用计数，本质上就是在碰 CPython 运行时。
...
最适合释放 GIL 的场景，不是“任何 C++ 代码”，而是：

```text
长时间 + 纯 C++ + 不碰 Python 对象
```
...
Python 调用函数
   ↓
pybind11 先解析参数（这时持有 GIL）
   ↓
进入 heavy_compute 前临时释放 GIL
   ↓
跑纯 C++ 计算
   ↓
结束后自动重新获取 GIL
...
所以释放 GIL 的真正价值不是“语法更漂亮”，而是：

```text
让其他 Python 线程在这段纯 C++ 工作期间继续运行
```
...
只要你的代码正在做下面任意一类事情，就不该把那一段放进“无 GIL 区”：

- 创建 `py::object`
- 操作 `py::list` / `py::dict`
- 访问 Python 属性
- 调 Python 函数 / Python 回调
- 操作依赖 CPython API 的异常或对象状态
- 让 `py::object` 在不安全位置析构
...
这个函数几乎全程都在碰 Python 对象，所以没有“值得释放”的纯 C++ 热区。
...
这里的 `f()` 本身就是 Python 调用，所以执行时必须持有 GIL。

### 一句判断法

```text
如果这段代码的核心动作是“操作 Python 世界”，就别释放 GIL。
```
...
真实工程里，很多函数既不是“全 Python”，也不是“全 C++”，而是混合型。

这时最稳的写法通常不是整段 release，而是：
...
这一段还在 Python / pybind11 边界层，所以保留 GIL 是自然的。
...
出了 release 作用域后，GIL 已自动拿回，所以回到 Python 是安全的。

### 这个例子的隐含前提

- 循环体内部不能去碰 Python 对象
- 你对底层共享内存的并发访问语义要有把握
- 如果其他线程也可能同时改这块内存，GIL 释放后不会替你兜底

这里一定要建立一个很牢的边界：

```text
GIL 保护的是 Python runtime，
不是你自己的 C++ 数据结构，也不是任意共享内存。
```
...
```text
“不要整段放掉，只把最热、最纯的那一段抠出来放掉。”
```

### 实战判断法

- 整个函数几乎全是纯 C++：优先 `py::call_guard<py::gil_scoped_release>()`
- 函数是混合型：优先局部 `py::gil_scoped_release`

这样做的好处是：

- 边界更清晰
- 审计起来更安全
- 不容易把 Python 操作误放进无 GIL 区
...
把整章内容最后再压成 6 条最实用的规则。

### 1) 默认保守：只有在明确的纯 C++ 热区才释放 GIL

不要为了“听说这样更快”就到处加 `gil_scoped_release`。

GIL 的正确优化姿势不是“多放几处”，而是：

```text
只放那些已经确认不碰 Python 的重计算区域
```
...
只要不是直接沿着 Python 调用栈进入的线程，就不要假设它天然安全。

碰 Python 前先 acquire，这是硬规则，不是建议。

---

### 4) 特别小心 `py::object` 的跨线程存活和析构

很多 bug 不是发生在“调用 Python”时，反而发生在：
...
```text
GIL 负责 Python runtime safety，
不负责你的业务并发正确性。
```

---

### 6) 回调 Python 前，尽量先释放你自己的 C++ 锁
...
所以这一章最重要的，不是背 API，而是先建立一个很稳的判断边界：

```text
有没有复制数据本体？
这块共享内存的 layout 和 owner 是否说清楚了？
```
...
以 NumPy array 为例，它本质上不是“只有一坨数据”，而是：
...
只要 pybind11 能把这些信息准确映射到 C++，C++ 就可以：
...
Python 世界里，很多对象都能暴露这样一件事：

```text
“我背后有一块原始内存，你可以按这个描述来读它。”
```
...
#### 1) `py::array_t<double, py::array::c_style>`
...
```text
“我要的就是这个 dtype 和 layout；不符合就拒绝。”
```

#### 3) `unchecked<1>()`

它拿到的是一个轻量 view，少掉了循环内部的重复检查开销。
...
- 如果传入的对象不是精确匹配的数组类型，pybind11 可能先构造一个临时兼容数组
...
- **教学/演示 API**：如果为了易用性保留 `forcecast`，文档里必须明确写“可能复制”

一句话：

```text
forcecast 提高了易用性，
但它也最容易把“零拷贝接口”悄悄变成“复制接口”。
```
...
反方向做零拷贝时，真正的难点通常已经不是 `dtype / strides`，而是：

```text
Python 返回去的 view 指向哪块 C++ 内存？
这块内存谁拥有？
它什么时候释放？
```

所以这一节最核心的，不是“怎么构造 ndarray 语法”，而是：
...
只把指针借出去、不把 owner 讲清楚，基本就是在埋悬空指针的雷。
...
这三类接口都可以存在，但不要把它们混成同一种语义。

---

### 2) 性能敏感接口默认不用 `forcecast`
...
### 3) 连续布局 API 和 stride-aware API 是两类设计

- 如果算法要求连续内存，就明确要求 `c_style`
- 如果算法能处理切片 / 转置 / view，就按 `shape + strides` 写循环

不要一边假设连续，一边又偷偷接收任意 strided 输入。
...
### 5) 返回 C++ 内存给 Python 时，一定显式绑定 owner
